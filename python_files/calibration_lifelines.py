...
def survival_probability_calibration(model: RegressionFitter, df: pd.DataFrame, t0: float, ax=None, left_y_axis=False, right_y_axis=False):
    r"""
    ...
    x = np.linspace(np.clip(predictions_at_t0.min() - 0.01, 0, 1), np.clip(0.75, 0, 1), 100)
